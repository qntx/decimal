...
func bintFromU64(u uint64) bint {
	return bint{u128: uint128.NewFromUint64(u)}
}
...
func parseSmallToU128(s []byte) (uint128.Uint128, uint8, error) {
	var (
		coef uint64
		prec uint8
	)
	...
	for i := range len(s) {
		if s[i] == '.' {
			...
			if prec > defaultPrec {
				return uint128.Zero, 0, ErrPrecOutOfRange
			}
	...
	if coef == 0 {
		return uint128.Zero, 0, nil
	}

	return uint128.NewFromUint64(coef), prec, nil
}

func parseLargeToU128(s []byte) (uint128.Uint128, uint8, error) {
	// find '.' position
	l := len(s)
	...
	default:
		return uint128.Zero, 0, fmt.Errorf("invalid parse mode: %d. Make sure to use SetParseMode with a valid value", defaultParseMode)
	}
	...
	// combine
	coef, err := intPart.Mul64(pow10[uint64(prec)].Low())
	if err != nil {
		return uint128.Zero, 0, err
	}

	coef, err = coef.Add64(fractionPart.Low())
	if err != nil {
		return uint128.Zero, 0, err
	}
...
func digitToU128(s []byte) (uint128.Uint128, error) {
	if len(s) <= maxDigitU64 {
		...
		return uint128.NewFromUint64(u), nil
	}
