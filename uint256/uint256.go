...
// Big returns *big.Int representation.
...
// NewFromDecimal converts decimal.Decimal to Uint256.
// It only considers the integer part of the decimal.
func NewFromDecimal(d decimal.Decimal) (Uint256, error) {
	return NewFromBigInt(d.BigInt())
}

// MustFromDecimal converts decimal.Decimal to Uint256, panicking on error.
func MustFromDecimal(d decimal.Decimal) Uint256 {
	u, err := NewFromDecimal(d)
	if err != nil {
		panic(err)
	}
	return u
...
// Pow returns base^exponent. It returns an error on overflow.
// If exponent is 0, it returns 1.
// If base is 0 and exponent is > 0, it returns 0.
func (base Uint256) Pow(exponent uint64) (Uint256, error) {
	if exponent == 0 {
		return One, nil
	}
	// For 0^exponent where exponent > 0
	if base.IsZero() {
		return Zero, nil
	}
	...
	tempExp := exponent
	for tempExp > 0 {
		if tempExp&1 == 1 { // If LSB is 1 (exponent is odd)
			result, err = result.Mul(currentPower)
			if err != nil {
				return Zero, err // Propagate overflow error
			}
		}
		tempExp >>= 1    // Halve the exponent
		if tempExp > 0 { // Only square if currentPower will be needed again
			currentPower, err = currentPower.Mul(currentPower)
			if err != nil {
				return Zero, err // Propagate overflow error
			}
		}
