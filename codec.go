...
func (d Decimal) appendBuffer(inBuf []byte, trimTrailingZeros bool, withQuote bool) []byte {
	...
	if d.prec == 0 {
		...
		quo, rem = d.coef.u128.QuoRem64(pow10[d.prec].Low()) // max prec is 19, so we can safely use QuoRem64
	}
	...
	if quo.IsZero() {
		...
		if quo.Cmp64(10) >= 0 {
			buf[n] = table[quo.Low()*2+1]
			buf[n-1] = table[quo.Low()*2]
			n -= 2
		} else {
			buf[n] = byte(quo.Low()) + '0'
			n--
		}
	}
...
func quoRem64(u uint128.Uint128, v uint64) (q uint128.Uint128, r uint64) {
	if u.High() == 0 {
		return uint128.NewFromUint64(u.Low() / v), u.Low() % v
	}
...
func (d Decimal) appendBinaryU128(input []byte) ([]byte, error) {
	...
	if coef.High() == 0 {
		buf = make([]byte, 11)
		buf[2] = 11
		copyUint64ToBytes(buf[3:], coef.Low())
	} else {
		buf = make([]byte, 19)
		buf[2] = 19
		copyUint64ToBytes(buf[3:], coef.High())
		copyUint64ToBytes(buf[11:], coef.Low())
	}
...
func (d *Decimal) unmarshalBinaryU128(data []byte) error {
	...
	case 19:
		hi := binary.BigEndian.Uint64(data[3:])
		lo := binary.BigEndian.Uint64(data[11:])
