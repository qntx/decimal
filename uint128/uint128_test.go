	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			for i := range iterations {
				...
				// Test BigInt/NewFromBig roundtrip
...
// assertBigRoundtrip verifies that BigInt and NewFromBig are inverses.
func assertBigRoundtrip(t *testing.T, x Uint128) {
	t.Helper()

	u, err := NewFromBigInt(x.BigInt())
	if err != nil {
...
func assertComparisons(t *testing.T, x, y Uint128) {
	...
	if got := x.Cmp(y); got != x.BigInt().Cmp(y.BigInt()) {
		t.Errorf("Cmp(%v, %v) = %v, want %v", x, y, got, x.BigInt().Cmp(y.BigInt()))
	}
	...
	// Compare with uint64
	if got := x.Cmp64(y.lo); got != x.BigInt().Cmp(NewFromUint64(y.lo).BigInt()) {
		t.Errorf("Cmp64(%v, %v) = %v, want %v", x, y.lo, got, x.BigInt().Cmp(NewFromUint64(y.lo).BigInt()))
	}
...
func TestArithmetic(t *testing.T) {
	...
	checkBinOpX := func(x Uint128, op string, y Uint128, fn func(x, y Uint128) Uint128, fnb func(z, x, y *big.Int) *big.Int) {
		...
		rb := fnb(new(big.Int), x.BigInt(), y.BigInt())
	...
	checkBinOp := func(x Uint128, op string, y Uint128, fn func(x, y Uint128) Uint128, fnb func(z, x, y *big.Int) *big.Int) {
		...
		r := fn(x, y)
		rb := mod128(fnb(new(big.Int), x.BigInt(), y.BigInt()))
	...
	checkShiftOp := func(x Uint128, op string, n uint, fn func(x Uint128, n uint) Uint128, fnb func(z, x *big.Int, n uint) *big.Int) {
		...
		r := fn(x, n)
		rb := mod128(fnb(new(big.Int), x.BigInt(), n))
	...
	checkBinOp64X := func(x Uint128, op string, y uint64, fn func(x Uint128, y uint64) Uint128, fnb func(z, x, y *big.Int) *big.Int) {
		...
		xb, yb := x.BigInt(), NewFromUint64(y).BigInt()
	...
	for range 1000 {
		...
		if y64 != 0 {
			checkBinOp64(x, "/", y64, Uint128.MustDiv64, (*big.Int).Div)

			modfn := func(x Uint128, y uint64) Uint128 {
				return NewFromUint64(x.MustMod64(y))
			}
...
func TestString(t *testing.T) {
	for range 1000 {
		x := randUint128()
		if x.String() != x.BigInt().String() {
			t.Fatalf("mismatch:\n%v !=\n%v", x.String(), x.BigInt().String())
		}
